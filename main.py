...
class LoadTestApp:
    def __init__(self, root):
        ...
        ttk.Label(root, text="GPU VRAM負荷（%）").grid(column=0, row=2, padx=10, pady=10)
        self.gpu_vram_slider = ttk.Scale(root, from_=0, to=100, variable=self.gpu_vram_load)
        self.gpu_vram_slider.grid(column=1, row=2, padx=10, pady=10)

        ttk.Label(root, text="GPU負荷タイプ").grid(column=0, row=3, padx=10, pady=10)
        ...
        self.start_button.grid(column=0, row=4, columnspan=3, pady=20)

        self.stop_button = ttk.Button(root, text="負荷を停止", command=self.stop_load)
        self.stop_button.grid(column=0, row=5, columnspan=3, pady=20)

        self.exit_button = ttk.Button(root, text="終了", command=self.exit_app)
        self.exit_button.grid(column=0, row=6, columnspan=3, pady=20)
        ...
        self.psu_power_label.grid(column=0, row=8, columnspan=3, pady=10)

        # CPUとメモリの使用率を表示するラベル
        self.cpu_usage_label = tk.Label(root, text="CPU Usage: N/A", font=("Helvetica", 14))
        self.cpu_usage_label.grid(column=0, row=9, columnspan=3, pady=10)

        self.memory_usage_label = tk.Label(root, text="Memory Usage: N/A", font=("Helvetica", 14))
        self.memory_usage_label.grid(column=0, row=10, columnspan=3, pady=10)

        # GPU VRAM使用率を表示するラベル
        self.gpu_vram_label = tk.Label(root, text="VRAM Usage: N/A", font=("Helvetica", 14))
        self.gpu_vram_label.grid(column=0, row=11, columnspan=3, pady=10)
    ...
    def apply_load(self):
        ...
        self.info_area.insert(tk.END, f"GPU VRAM負荷: {gpu_vram_percentage}%\n")
    ...
    def update_system_info(self):
        while True:
            ...
            if vram_usage > 90:  # 90%を超えたら警告を表示
                messagebox.showwarning("警告", "VRAM使用量が90%を超えました。負荷を減らしてください。")
